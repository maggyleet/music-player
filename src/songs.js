import lifeOfRam from './assets/Life-of-Ram.mp3';
import nenjukulle from './assets/Nenjukulle.mp3';
import mayaNadhi from './assets/Maya-Nadhi.mp3';
import heySinamika from './assets/Hey_Sinamika.mp3'
import journey from './assets/Journey.mp3'
import moongilThottam from './assets/Kadal_Moongil_Thottam.mp3'
import laaliLaali from './assets/Laali_Laali.mp3'
import marudhaani from './assets/Marudhaani.mp3'
import kodiAruvi from './assets/kodiAruvi.mp3'
import neethanae from './assets/Neethaanae_Mersal.mp3'
import naan_un from './assets/Naan_un.mp3'
import nilaneevaanam from './assets/Nila-Nee-Vaanam.mp3'
import agastha from './assets/Agasatha.mp3'
import lifeOfPazham from './assets/Life-of-Pazham.mp3'

const songs = [
    {
        title: "Life of Ram - 96",
        artist: "Pradeep Kumar, Govind Vasantha",
        src: lifeOfRam,
    },
    {
        title: "Nenjukulle - Kadal",
        artist: "Shakthishree Gopalakrishnan, A R Rahman",
        src: nenjukulle,
    },
    {
        title: "Maya Nadhi - Kabali",
        artist: "Pradeep Kumar, Swetha Mohan, Santhosh Narayanan",
        src: mayaNadhi,
    },
    {
        title: "Thozhi - Hey Sinamika",
        artist: "Pradeep Kumar, Govind Vasantha",
        src: heySinamika,
    },
    {
        title: "Journey - 96",
        artist: "Pradeep Kumar",
        src: journey,
    },
    {
        title: "Moongil Thottam - Kadal",
        artist: "Abhay Jodhpurkar, A R Rahman, Harini",
        src: moongilThottam,
    },
    {
        title: "Laali Laali",
        artist: "Sathyaprakash, Pragathi Guruprasad, Ghibran",
        src: laaliLaali,
    },
    {
        title: "Marudhaani",
        artist: "Madhushree, A R Rahman",
        src: marudhaani,
    },
    {
        title: "Naan Un - 24",
        artist: "Arjit Singh, Chinmayi Sripada, A R Rahman",
        src: naan_un,
    },
    {
        title: "Kodi Aruvi",
        artist: "Pradeep Kumar, Nithyasree Venkatramanan, Sean Roldan",
        src: kodiAruvi,
    },
    {
        title: "Neethanae - Mersal",
        artist: "Shreya Ghosal, A R Rahman",
        src: neethanae,
    },
    {
        title: "Nila Nee Vaanam",
        artist: "Vijay Yesudas, Chinmayi Sripada",
        src: nilaneevaanam,
    },
    {
        title: "Agasatha - Cuckoo",
        artist: "Pradeep Kumar, Kalyani Nair, Santhosh Narayanan",
        src: agastha,
    },
    {
        title: "Life Of Pazham",
        artist: "Anirudh Ravichandran, Dhanush",
        src: lifeOfPazham,
    },
];

export default songs;
